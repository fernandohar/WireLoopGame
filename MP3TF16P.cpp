...
bool MP3TF16P::checkReturn() {
	...
	#else
    	if (_enableDebug) _dbg->println("MP3 checkReturn");
	#endif		
...
void MP3TF16P::setAmplification(boolean turnOn, byte gain) {
    if (gain > 31) gain = 31;
	resetCommandBuffer();
	_buf[3] = 0x10;
	_buf[5] = (turnOn)? 0x01 : 0x00;
	_buf[6] = gain;
	sendCommand();    
}
...
uint8_t MP3TF16P::getPlayStatus() {
	resetCommandBuffer();
	_buf[3] = 0x42;
	_buf[4] = 0x01;
	sendCommand(true);
	if (_retCnt == 10) {
		if ((_retBuf[0] == 0x7E) && (_retBuf[1] == 0xFF) && (_retBuf[2] == 0x06) && (_retBuf[3] == 0x41) && (_retBuf[9] == 0xEF)) {
			return _retBuf[6];
		}
	}
	return 0xFF;
